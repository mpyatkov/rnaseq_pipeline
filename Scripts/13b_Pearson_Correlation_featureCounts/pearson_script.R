...
if(!is.na(list.filenames.HT[1])){
  ...
  # f <- paste0("Pearson_All",file1,".pdf")
   ...
   # save PCA plot Raw
   ggsave("PCA_All_Merge_File.pdf",plot= pca, device = "pdf",path= wd, width = 80, height =40, units = "cm")
  ...
  # filtering of the samples.
   ...
   rownames(df2) <- str_remove(rownames(df2), "rpkm_")   #remove the rpkm from colnames for plot
   ...
   pca_FC <- autoplot(prcomp(df_FC), data= df2, colour= "label", label = TRUE, label.size = 3.5)
   ...
   rownames(df3) <- str_remove(rownames(df3), "rpkm_")   #remove the rpkm from colnames for plot
   ...
   # PCA plot
   pca_FC2 <- autoplot(prcomp(df_FC2), data= df3, colour= "label", label = TRUE, label.size = 3.5)
