...
list.filenames.HT
list.filenames.HT_prefix <- str_remove(list.filenames.HT, "DiffExp_v2_LncRNA_ExonCollapsed_")
list.filenames.HT_prefix <- str_remove(list.filenames.HT_prefix, "_featureCounts.txt")
...
if(!is.na(list.filenames.HT[1])){
   ...
   # PCA plot
   pca <- autoplot(prcomp(df1), data= df, colour= "label", label = TRUE, label.size = 3)
   ...
   tsneplot <- ggplot(d_tsne_1, aes(x=V1, y=V2), colour="green") +  
   ...
   tsneplot <- tsneplot + geom_label_repel(aes(label = rownames(df1)),
                                           box.padding   = 0.35, 
                                        point.padding = 0.5,
 ...
 # correlation
  ...
  # f <- paste0("Pearson_All",file1,".pdf")
   ...
   # tsne plot save
  ggsave("tSNE_All_Merge.pdf",plot= tsneplot, device = "pdf",path= wd, width = 60, height =30, units = "cm")
  ...
  # filtering of the samples.
   ...
   pca_FC <- autoplot(prcomp(df_FC), data= df2, colour= "label", label = TRUE, label.size = 3)
   ...
   # PCA plot
   pca_FC2 <- autoplot(prcomp(df_FC2), data= df3, colour= "label", label = TRUE, label.size = 3)
