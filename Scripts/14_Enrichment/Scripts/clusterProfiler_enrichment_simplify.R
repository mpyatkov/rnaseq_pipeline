...
construct_title <- function(filename, top, stats_title) {
  ...
  str_glue("{comparison_num}: {top} (clusterProfiler {packageVersion('clusterProfiler')}) SIMPLIFIED\n",
           "{updown}, {stats_title}\n",
           "{body}")
}
...
# goterm_analysis_mf_orig <- enrichGO(gene = entrez_ids,OrgDb = mmdb, ont = "MF", pvalueCutoff  = 0.05, readable = TRUE)
...
goterm_analysis_run <- function(entrez_ids, db, ontology, pcutoff = 0.05) {
  analysis <- enrichGO(entrez_ids, OrgDb = db, ont = ontology, pvalueCutoff=pcutoff, readable = TRUE)
  if (is.null(analysis) || nrow(analysis@result) == 0) return(analysis)
  mmBP <- godata(annoDb = db, ont = ontology, computeIC = FALSE)
  clusterProfiler::simplify(analysis, semData = mmBP)  
}

goterm_analysis_bp <- goterm_analysis_run(entrez_ids, mmdb, "BP", pcutoff = 0.05)
goterm_analysis_mf <- goterm_analysis_run(entrez_ids, mmdb, "MF", pcutoff = 0.05)
goterm_analysis_cc <- goterm_analysis_run(entrez_ids, mmdb, "CC", pcutoff = 0.05)

# mmBP <- godata(annoDb = mmdb, ont = "BP")
# goterm_analysis_bp <- enrichGO(entrez_ids, OrgDb = mmdb, ont = "BP", pvalueCutoff=0.05)
# goterm_analysis_bp <- clusterProfiler::simplify(goterm_analysis_bp, semData = mmBP)
# goterm_analysis_bp@result |> glimpse()
...
outputnames_ontologies <- paste0(str_pad(5:7, width = 2, side = "left", pad = "0"),
                      "_",
                      compar_num,
                      "_",
                      c("MF", "BP", "CC"),
...
walk2(flatten(list(plots,treeplots)), names(all_plots), \(p,name) {
  name <- tools::file_path_sans_ext(name) %>% paste0("_simplified.pdf")
  ggsave(plot = p, filename = name, width = 13, height = 15.19)
})


output_fname_xlsx <- basename(fname) %>% tools::file_path_sans_ext()  %>%  paste0("_simplified.xlsx") %>% paste0(compar_num,"_",.)
